...
public class CustomerOrderServiceTest extends IntegrationTestPlatform {
    ...
    public void canPlaceValidOrder() throws Exception {
        ...
        assertObjectStoredByKey(FIND_CUSTOMER_BY_EMAIL, validEmail, JDBCType.VARCHAR,
            ()->String.format("Could not locate customer by email %s", validEmail),
            (ResultSet resultSet) -> {
                try {
        ...
        assertObjectStoredById(FIND_CUSTOMER_ORDER_BY_ID_SQL,
            customerOrderId,
            ()->String.format("Could not locate customer order %d", customerOrderId),
            (ResultSet resultSet) -> {
                try {
                    ...
                    assertAll("Customer Order read looks valid",
                        ...
                        () -> assertTrue(now.getTime() - dateCreated.getTime() < TimeUnit.SECONDS.toMillis(30),
                            "Expected created timestamp close to the current time"),
        ...
        assertObjectStoredById(FIND_CUSTOMER_ORDER_LINE_BY_ID, customerOrderId,
            ()->String.format("Could not locate customer order line by id %s", customerOrderId),
            (ResultSet resultSet) -> {
                try {
                    Long dbCustomerId = resultSet.getLong("customer_id");
                    String dbCustomerName = resultSet.getString("name");
